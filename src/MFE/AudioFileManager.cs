...
namespace MFE
{
    public static class AudioFileManager
    {
        ...
        public static Task<bool> CheckForOvermodulation(IProgress<ProgressInfo> progress, double factor, IEnumerable<string> ReferenceFiles)
        {
            return Task.Run(() =>
            {
                ProgressInfo info = new ProgressInfo { Progression = 0f };
        ...
        /// <returns></returns>
        public static Task AdjustFiles(IProgress<ProgressInfo> progress, double factor, IEnumerable<string> ReferenceFiles, Func<string, string> NewPaths, Action<string> fileSucceed, Action<string, Exception> fileFailed)
        {
            return Task.Run(() =>
            {
                ProgressInfo info = new ProgressInfo { Progression = 0f };
